...
# 在你的config模块中定义log_interval或者直接在脚本中设置

def train_epoch(train_loader, model, optimizer, scheduler, epoch, total_steps,dev_loader):
    ...
    for idx, batch_samples in enumerate(train_loader):
        ...
        current_step = (epoch - 1) * len(train_loader) + idx + 1

        # Print or log the loss for every 10% of the epoch
        if (idx + 1) % int(len(train_loader) / 10) == 0:
            average_loss = train_losses / (idx + 1)
            logging.info(f"Step {current_step}/{total_steps}, Average Batch Loss: {average_loss}")
            # Evaluate the model on the validation set
            val_metrics = evaluate(dev_loader, model)
            logging.info(f"Step {current_step}/{total_steps}, Validation F1: {val_metrics['f1']}")
    ...
    logging.info("Epoch: {}, Average train loss: {}".format(epoch, train_loss))
...
def train(train_loader, dev_loader, model, optimizer, scheduler, model_dir, local_rank):
    ...
    for epoch in range(1, config.epoch_num + 1):
        train_epoch(train_loader, model, optimizer, scheduler, epoch,total_steps,dev_loader)
        val_metrics = evaluate(dev_loader, model)
...
def evaluate(dev_loader, model, mode='dev'):
    ...
    with torch.no_grad():
        for idx, batch_samples in enumerate(dev_loader):
            ...
            if mode == 'dev':
                batch_output = model.crf.decode(batch_output, mask=label_masks)#module.
            else:
