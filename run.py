...
def test():
    ...
    if config.model_dir is not None:
        ...
        #model = BertSeg(config)
        model=torch.load(config.model_dir+"/model.pth")
...
def run():
    ...
    # build data_loader
    train_loader = DataLoader(train_dataset, batch_size=config.batch_size, shuffle=True, #sampler=Sampler(train_dataset),
                              collate_fn=train_dataset.collate_fn, num_workers=4)
    dev_loader = DataLoader(dev_dataset, batch_size=config.batch_size, shuffle=False,#sampler=Sampler(dev_dataset),
    ...
    model = DualBertSeg(config)
    # 要先将model放到gpu上
    model = model.to(device)
    # Prepare optimizer
    if config.full_fine_tuning:
        # model.named_parameters(): [bert, classifier, crf]
        bert_optimizer = list(model.bert_model.named_parameters())
        our_optimizer = list(model.our_model.named_parameters())
        #geobert_optimizer = list(model.geobert_model.named_parameters())
        #print(bert_optimizer)
        ...
        optimizer_grouped_parameters = [
            ...
            {'params': [p for n, p in bert_optimizer if any(nd in n for nd in no_decay)],
             'weight_decay': 0.0},
             {'params': [p for n, p in our_optimizer if not any(nd in n for nd in no_decay)],
             'weight_decay': config.weight_decay},
            {'params': [p for n, p in our_optimizer if any(nd in n for nd in no_decay)],
             'weight_decay': 0.0},
...
if __name__ == '__main__':
    ...
    logging.info(f"Total training time: {elapsed_time} seconds")